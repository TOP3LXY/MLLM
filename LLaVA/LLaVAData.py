...
class TrainLLaVAModelCollator:
    ...
    def __call__(self, features:List) -> torch.Any:
        ...
        for feature in features:
            qaimg = builde2tensor(processor=self.processor, q_text=feature[0], a_text=feature[1], image_path=feature[2])
            input_ids, labels = self.convert_one_piece(q_input_idx=qaimg.q_input_ids, a_input_idx=qaimg.a_input_ids)
        ...
        final_input_ids = torch.concat([
            ...
            value], axis=1) for index, value in enumerate(input_ids_list)  
        ] )
        ...
        final_labels = torch.concat([
            ...
            value], axis=1) for index, value in enumerate(labels_list)   
        ] )
